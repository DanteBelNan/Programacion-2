...
class Cpunto1{
    ...
    public:
        ...
        //Si yo quisiese que este operador siga la norma estandar de lo que hace el == (comparar) deberia utilizar un metodo como este
        /*

        bool operator == (Cpunto1 obj){
            if(this->codigoRestaurante != obj.getCodigoRestaurante()) { return false;}
            if(this->DNI != obj.getDNI()) { return false;}
            if(this->fechaReserva != obj.getFechaReserva()) { return false;}

            return true;

        }
        */
        //Pero, debido a que no hay un operator == para comparar las fechas, y crei una perdida de tiempo comparar
        //mes dia y año de cada fecha, decidi darle un proceso de asignación a este operador
